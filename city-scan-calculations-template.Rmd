...
if (length(pop) > 0) {
  print(paste("pop:", scales::label_comma()(pop * 1000)))
} else {
...
wsf <- read_xlsx("Stats/WSF.xlsx", sheet = "WSF") %>%
  select(Year = year, uba_km2 = any_of(c("AREA_sq_km", "cumulative sq km"))) %>%
...
print(paste0("The city's built-up area grew from ", round(first_area, 2), " km^2 in ", first_year, " to ", round(latest_area, 2), " in ", latest_year, " for ", round(pct_growth, 2), "% growth"))
...
### Landcover
```{r, echo = F}
lc <- read_xlsx("Stats/Landcover.xlsx", skip = 1,
...
print(paste("In", fu[1,1], ",", round(fu[1,2], 2), "km^2 of the built-up area was exposed to river flooding at a minimum depth of 15 cm"))
...
paste("In", pu[1,1], ",", round(pu[1,2], 2), "km^2 of the built-up area was exposed to surface water flooding at a minimum depth of 15 cm")
...
print(paste("In", comb[1,1], ",", round(comb[1,2], 2), "km^2 of the built-up area was exposed to river or surface water flooding at a minimum depth of 15 cm"))
...
eq_text <- eq %>%
  ...
  mutate(
    ...
    day = case_when(is.na(day) ~ 1, T ~ day),
...
if (nrow(missing_plots) > 0) {
  print("The following plots have not been made. Why not?")
  print(missing_plots %>% select(slide, filename, warning_text))
}
```