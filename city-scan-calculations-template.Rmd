...
if (nrow(occasional_maps > 0)) {
  cat("These maps exist. Make sure they are included in the InDesign.")
...
if (nrow(missing_maps) > 0) {
  cat("The following maps are not included. Should they be?")
...
# Get area
cat(paste("AOI Area:", round(area, 1), "sq km"))
...
cat(paste("Köppen climate classification:", koeppen_city))
```
...
if (length(pop) > 0) {
  cat(paste("pop:", scales::label_comma()(pop * 1000)))
} else {
...
cat("Nearby non-benchmark cities")
oxford_full %>%
  select(Location, Country, Indicator, `2021`) %>%
...
cat("Yearly population, sorted by absolute growth")
pop_growth %>% mutate(growth = Population - lag(Population)) %>%
  arrange(desc(growth)) 
cat("Yearly population, sorted by percentage growth")
...
pop_dist_group %>% mutate(cumpct = cumsum(Percentage))
...
cat(paste("under5:", under5$Percentage %>% {scales::label_percent()(.)}))
cat(paste("youth (15-24):", youth$Percentage %>% {scales::label_percent()(.)}))
cat(paste("working_age (15-64):", working_age$Percentage %>% {scales::label_percent()(.)}))
cat(paste("elderly (60+):", elderly$Percentage %>% {scales::label_percent()(.)}))
cat(paste("reproductive_age, percent of women (15-50):", reproductive_age$Sexed_Percent %>% {scales::label_percent()(.)}))
cat(paste("sex_ratio:", round(sex_ratio, 2), "males to 100 females"))
}
```
...
if (in_oxford) {
  ...
  cat("Population growth from 2000 to 2021")
  pop_growth2
  ...
  cat("Employment growth from 2000 to 2021")
  emp_growth
  ...
  gdp_growth_years <- oxford %>% subset(Indicator == "GDP, real, US$ - Total") %>%
    ...
    group_by(Group, Location) %>%
    mutate(Growth = Value / lag(Value) - 1)
  cat("GDP growth from 2000 to 2021")
  gdp_growth
  # filter(gdp_growth_years, Year == 2022) %>% arrange(desc(Growth))
  cat("GDP growth in focus city for each year")
...
cat(paste0("The city's built-up area grew from ", round(first_area, 2), " km^2 in ", first_year, " to ", round(latest_area, 2), " in ", latest_year, " for ", round(pct_growth, 2), "% growth"))
...
cat(paste0(lc1, " comprises the majority of land in the ", city, " area of interest (", lc1_pct, "%)."))
cat(paste0("The next largest land classes are ", lc2, " (", lc2_pct, "%) and ", lc3, " (", lc3_pct, "%)"))
```
...
flood_string("fu")
fu
bind_rows(head(fu, 1), tail(fu, 1))
cat(paste("In", fu[1,1], ",", round(fu[1,2], 2), "km^2 of the built-up area was exposed to river flooding at a minimum depth of 15 cm"))
cat(paste("Percentage of highest populated area exposed to fluvial flooding:", flood_pop_area("fu")$Percentage %>% scales::percent()))
}
```
...
flood_string("pu")
pu
bind_rows(head(pu, 1), tail(pu, 1))
cat(paste("In", pu[1,1], ",", round(pu[1,2], 2), "km^2 of the built-up area was exposed to surface water flooding at a minimum depth of 15 cm"))
cat(paste("Percentage of highest populated area exposed to pluvial flooding:", flood_pop_area("pu")$Percentage %>% scales::percent()))
...
bind_rows(head(comb, 1), tail(comb, 1))
cat(paste("In", comb[1,1], ",", round(comb[1,2], 2), "km^2 of the built-up area was exposed to river or surface water flooding at a minimum depth of 15 cm"))
cat(paste("Percentage of highest populated area exposed to combined flooding:", flood_pop_area("comb")$Percentage %>% scales::percent()))
}
```
...
if (nrow(missing_plots) > 0) {
  cat("The following plots have not been made. Why not?")
  missing_plots %>% select(slide, filename, warning_text)
}
```