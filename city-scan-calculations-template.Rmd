...
# City Scan Calculations

## User-defined inputs

```{r, echo = F}
if (!dir.exists("plots")) dir.create("plots") else message(paste("plots/ directory already exists in", getwd()))

# Select Cities
city <- "Can Tho"
country <- "Vietnam"
bm_cities_manual <- c("Ca Mau", "Cao Lanh", "Hong Ngu", "Rach Gia", "Hoi An", "Yen Bai", "Cao Bang", "Dien Bien Phu") # NULL when not in use
map_file_prefices <- c("'Can_Tho_'", "'Cantho_'") # Include single quotations around prefices
...
if (nrow(missing_maps) > 0) {
  print("The following maps are not included. Should they be?")
  print(missing_maps %>% select(slide, filename, warning_text))
}
...
# Get area
print(paste("AOI Area:", round(area, 1), "sq km"))

# Climate
coords <- st_centroid(aoi) %>% st_coordinates() %>% suppressWarnings()
names(coords) <- c("Lon", "Lat")       
koeppen <- read_csv("../../03-multi-scan-materials/Koeppen-Geiger-ASCII.csv", col_types = "ddc")
...
print(paste("Köppen climate classification:", koeppen_city))
```

```{r}
# Defining the UN Data and citypopulation.de pop function because it is used as backup in Oxford and in Density
  get_un_de_pop_growth <- function(city) {
    # UN Data
    pop_growth_undata <- read_csv("../../03-multi-scan-materials/undata-pop.csv",
    ...
    pop_growth_de <- de %>%
      select(Location = Name, contains("Population"), "Area") %>%
      mutate(Area_km = Area/100, .keep = "unused") %>%
    ...
    if (nrow(pop_growth_de) == 0) warning("No population data detected in citypopulation.de table")
    ...
    pop_growth <- bind_rows(pop_growth_de, pop_growth_undata)
    return(pop_growth)
  }
```
...
# For no
# Read Data ----
oxford_full <- read_csv("../../03-multi-scan-materials/Oxford Global Cities Data.csv", col_types = "cccccccccdddddddddddddddddddddddddddddddddddddddddcllldlcclcc")
...
pop <- subset(oxford_full, Location == city & Indicator == "Total population", select = `2021`) %>% pull()
if (length(pop) > 0) {
paste("pop:", scales::label_comma()(pop * 1000))
} else {
  pop_growth_un_de <- get_un_de_pop_growth(city)
...
bm_cities <- c(bm_cities, bm_cities_manual) %>% unique()
...
print("Nearby non-benchmark cities")
oxford_full %>%
  select(Location, Country, Indicator, `2021`) %>%
...
if (nrow(pop_growth) == 0) {
  warning("No population data from UN or citypopulation.de")

# Other data source, manually downloaded/created
  pop_growth <- read_xlsx("PATH.xlsx", sheet = 1) %>%
    # mutate(Source = "Turkish Stastistical Institute") %>%
    subset(City == city & !is.na(Population)) %>% arrange(Year)
...
pop_growth_plot <- ggplot(pop_growth, aes(x = Year, y = Population)) + #, color = Source))
  geom_line() +
  geom_point() +  
  scale_x_continuous(minor_breaks = seq(pop_min_year, pop_max_year, 1)) + 
...
avg_growth <- pop_growth %>% mutate(growth_pct = (Population - lag(Population))/lag(Population)) %>% arrange(desc(growth_pct)) %>% .$growth_pct %>% mean(na.rm = T) %>% round(4)
...
print("Yearly population, sorted by absolute growth")
pop_growth %>% mutate(growth = Population - lag(Population)) %>%
  arrange(desc(growth)) %>% print()
print("Yearly population, sorted by percentage growth")
pop_growth %>% mutate(growth_pct = (Population - lag(Population))/lag(Population)) %>%
  arrange(desc(growth_pct)) %>% print()
if (in_oxford) {
  print(paste("Average growth rate 2000-2010:",
  pop_growth %>% mutate(growth_pct = (Population - lag(Population))/lag(Population)) %>%
...
density <- subset(oxford, Indicator == "Total population") %>%
  ...
  # bind_rows( # For if city is not in Oxford
  #   data.frame(Location = city, Group = city, Population = pop_last)
  # )
  # bind_rows(
  #   read_xlsx("../osmaniye-balikesir-pop-formatted.xlsx", sheet = 1) %>%
  #     mutate(Source = "Turkish Stastistical Institute") %>%
  #     group_by(City) %>% slice_max(Year) %>%
  #     mutate(Location = City, Population = Population, Group = case_when(
  #       Location == city ~ city,
  #       T ~ "Benchmark"
  #     ), .keep = "none")
  # )

# oxford_shapefiles <- read_sf("/Users/bennotkin/Documents/world-bank/crp/city-scans/03-multi-scan-materials/OE_FUA_SHAPEFILE")
# oxford_shapefiles <- oxford_shapefiles %>% st_make_valid()
# # which(!st_is_valid(oxford_aoi))
# oxford_areas <-
#   data.frame(
#     Location = oxford_shapefiles %>% st_drop_geometry() %>% .$OE_FUANAME,
#     Country = oxford_shapefiles %>% st_drop_geometry() %>% .$OE_COUNTRY,
#     Area_km = units::set_units(st_area(oxford_shapefiles), km^2))
# write.csv(oxford_areas, "/Users/bennotkin/Documents/world-bank/crp/city-scans/03-multi-scan-materials/oxford-economics-areas.csv", row.names = F)

bm_areas <- read_csv("../../03-multi-scan-materials/oxford-economics-areas.csv", col_types = "ccd") %>%
...
# curl::curl_download(paste0("https://geodata.ucdavis.edu/gadm/gadm4.1/json/gadm41_", name2iso(country), "_2.json.zip", "tmp-admins.zip")
# unzip("tmp-admins.zip")
# filename <- unzip("tmp-admins.zip", list = T) %>% .$Name
# gadm <- st_read(filename)
# file.remove("tmp-admins.zip")
# file.remove(filename)
# gadm
# filter(gadm, NAME_2 %in% bm_cities | VARNAME_2 %in% bm_cities)

# library(osmdata)
# places <- getbb("Turkey", featuretype = "country") %>%
#   opq() %>%
...
# muni <- getbb("Turkey", featuretype = "country") %>%
#   opq() %>%
#   add_osm_features(features = c(
#     '"place"="municipality"')) %>%#,
#     # '"place"="town"')) %>%
...
# # Use cat(bm_cities, sep = '" = \n"') to set up named vector
# cat(bm_cities, sep = '" = \n"')
# area_km <- c(
#   "Adana" = 1950,
#   "Adapazari" = NULL,
...
#   "Balıkesir" = 43
#   )
# # area_km <- area_miles * 2.59

# density$Area_km <- area_km[density$Location]
...
non_oxford_pop <- which_not(c(city, bm_cities), oxford$Location) %>%
  lapply(get_un_de_pop_growth) %>%
  bind_rows() %>%
  mutate(Location = str_extract(tolatin(Location), bm_cities %>% paste(collapse = "|")))
...
print(pop_dist_group %>% mutate(cumpct = cumsum(Percentage)))
...
print(paste("under5:", under5$Percentage %>% {scales::label_percent()(.)}))
print(paste("youth (15-24):", youth$Percentage %>% {scales::label_percent()(.)}))
print(paste("working_age (15-64):", working_age$Percentage %>% {scales::label_percent()(.)}))
print(paste("elderly (60+):", elderly$Percentage %>% {scales::label_percent()(.)}))
print(paste("reproductive_age, percent of women (15-50):", reproductive_age$Sexed_Percent %>% {scales::label_percent()(.)}))
print(paste("sex_ratio:", round(sex_ratio, 2), "males to 100 females"))
```

### Age structure

Back to Oxford Economics
...
if (in_oxford) {
  ...
  print("Population growth from 2000 to 2021")
  print(pop_growth2)
  ...
  print("Employment growth from 2000 to 2021")
  print(emp_growth)
  emp_growth_years <- oxford %>% subset(Indicator == "Employment - Total") %>%
    ...
    group_by(Group, Location) %>%
    mutate(Growth = Value / lag(Value) - 1)
  filter(emp_growth_years, Location == city)
  ...
  gdp_growth_years <- oxford %>% subset(Indicator == "GDP, real, US$ - Total") %>%
    ...
    group_by(Group, Location) %>%
    mutate(Growth = Value / lag(Value) - 1)
  print("GDP growth from 2000 to 2021")
  print(gdp_growth)
  # filter(gdp_growth_years, Year == 2022) %>% arrange(desc(Growth))
  print("GDP growth in focus city for each year")
  filter(gdp_growth_years, Location == city)
...
print("Total pop in 2021")
print(pop_longitude %>% slice_max(Year) %>% arrange(-Population) %>% select(Group, Location, Population))
...
print("Total employed in 2021")
print(emp_longitude %>% slice_max(Year) %>% arrange(-`Total employed`) %>% select(Group, Location, `Total employed`))

ggplot(emp_longitude) +
  ...
  geom_text_repel(data = subset(emp_longitude, Year == 2021), aes(label = Location, x = 2021, y = `Total employed`, color = Group), 
                  hjust = 0, size = 3, xlim = c(2021.25, 2030), box.padding = 0.05, direction = "y")
ggsave("plots/oxford-emp_line.png", device = "png",
       width = 5, height = 4.6, units = "in", dpi = "print")
...
print("Total GDP in 2021")
print(gdp_longitude %>% slice_max(Year) %>% arrange(-GDP) %>% select(Group, Location, GDP))

ggplot(gdp_longitude) +
...
ggsave("plots/oxford-gdp-per-capita.png", device = "png",
       width = 8, height = 5, units = "in", dpi = "print")
subset(gdppc, Location == city)
...
incomes %>% group_by(Location)  %>%
  ...
  labs(title = "Household income distribution (PPP constant 2015 dollars)",
       ...
       y = "Income Group")
ggsave("plots/oxford-household-incomes-bar.png", device = "png",
       width = 8, height = 5, units = "in", dpi = "print")
...
ggplot(incomes_city2) +
  ...
  labs(title = paste0("Household income distribution, ", min(incomes_city2$Year), "-2021 (PPP constant 2015 dollars)"),
       ...
       x = "Year")
ggsave("plots/oxford-household-incomes-area.png", device = "png",
...
```{r, echo = F}
# Urban built-up area ----
wsf <- read_xlsx("Stats/WSF.xlsx", sheet = "WSF") %>%
  select(Year = year, uba_km2 = AREA_sq_km) %>%
...
print(paste0("The city's built-up area grew from ", first_area, " km^2 in ", first_year, " to ", latest_area, " in ", latest_year, " for ", pct_growth, "% growth"))
...
### Landcover
```{r, echo = F}
lc <- read_xlsx("Stats/Landcover.xlsx") %>%
  rename(percent = `...4`) %>% 
  remove_missing() %>%
  rename(`Land Cover` = OID_, Percent = percent) %>%
...
ggdonut <- function(data, category, percent_col, colors, title) {
  ...
  p <- ggplot(data, aes_string(ymax="max", ymin="min", xmax=4, xmin=3, fill= category)) +
...
ggdonut(lc, "`Land Cover`", "Percent", colors = lc_colors, title = "Land Cover")
ggsave("plots/wsf-lc-pieplot.png", device = "png",
       width = 8, height = 5, units = "in", dpi = "print")
...
print(paste0(lc1, " comprises the majority of land in the ", city, " area of interest (", lc1_pct, "%)."))
print(paste0("The next largest land classes are ", lc2, " (", lc2_pct, "%) and ", lc3, " (", lc3_pct, "%)"))
```

### Elevation
```{r, echo = F}
elevation <- read_xlsx("Stats/Elevaton_Slope.xlsx", sheet = "Elevation") 
...
### Slope
```{r, echo = F}
slope <- read_xlsx("Stats/Elevaton_Slope.xlsx", sheet = "Slope") 
...
flood_string <- function(flood_type) {
  ...
  major_roads <- read_xls(paste0("Stats/", flood_type, "_majorroad.xls"))
...
print(flood_string("fu"))
print(fu)
print(bind_rows(head(fu, 1), tail(fu, 1)))
print(paste("In", fu[1,1], ",", fu[1,2], "km^2 of the built-up area was exposed to river flooding at a minimum depth of 15 cm"))
print(paste("Percentage of highest populated area exposed to fluvial flooding:", flood_pop_area("fu")$Percentage %>% scales::percent()))
```
...
print(flood_string("pu"))
print(pu)
print(bind_rows(head(pu, 1), tail(pu, 1)))
paste("In", pu[1,1], ",", pu[1,2], "km^2 of the built-up area was exposed to surface water flooding at a minimum depth of 15 cm")
print(paste("Percentage of highest populated area exposed to pluvial flooding:", flood_pop_area("pu")$Percentage %>% scales::percent()))
...
print(flood_string("comb"))
# comb %>% mutate(diff = comb$uba_km2 - lag(comb$uba_km2)) %>% .$diff %>% mean(na.rm = T) 
print(bind_rows(head(comb, 1), tail(comb, 1)))
print(paste("In", comb[1,1], ",", comb[1,2], "km^2 of the built-up area was exposed to river or surface water flooding at a minimum depth of 15 cm"))
print(paste("Percentage of highest populated area exposed to combined flooding:", flood_pop_area("comb")$Percentage %>% scales::percent()))
```

## Flood Events
```{r, echo = F}
flood_archive <- read_sf("~/Documents/world-bank/crp/city-scans/03-multi-scan-materials/flood-archive")
aoi <- read_sf("AOI")
...
print(summarize(floods, count = n(), across(.cols = c(DEAD, DISPLACED), sum), .groups = "drop"))
print("Median flood event")
print(floods %>% mutate(
    duration = as.numeric(ENDED - BEGAN)) %>%
  summarize(across(.cols = c(DEAD, DISPLACED, duration), median), .groups = "drop"))

print("Causes of flood events")
print(count(floods, MAINCAUSE))
flood_text <- floods %>% mutate(
  ...
  line2 = paste(severity, tolower(MAINCAUSE), sep = ", "),
...
print(flood_text %>% select(BEGAN, text))
...
## Earthquakes
```{r, echo = F}
get_earthquake_data <- function(country) {
  library(httr)
  COUNTRY <- toupper(country) # because `country` is also a column name
...
eq <- get_earthquake_data(country)
...
ggplot(eq_text) +
  ...
  geom_path(data = eq_lines, aes(x = node_x, y = node_y, group = BEGAN))  +
  geom_hline(yintercept = 0, linetype = "dotted") + 
