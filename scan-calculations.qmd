...
stats_dir <- file.path(process_output_dir, "spatial")
output_dir <- file.path(city_dir, "03-render-output/")
data_sharing_dir <- file.path(output_dir, "data-sharing")
...
c("plots", file.path(data_sharing_dir, c(
    "02-TIFs", "03-analysis-inputs/flood-events",
    "03-analysis-inputs/earthquakes", "04-tabular-data"))) %>%
  lapply(function(directory) {
    if (!dir.exists(directory)) dir.create(directory, recursive = T) else {
      # message(paste0(directory, "/ directory already exists in ", getwd()))
}
  }) %>% invisible()

if (!any(dir.exists(file.path(data_sharing_dir, c("AOI", "01-AOI"))))) {
  file.copy(from = file.path(user_input_dir, "AOI"), to = data_sharing_dir, recursive = T)
  file.rename(file.path(data_sharing_dir, "AOI"), file.path(data_sharing_dir, "01-AOI"))
}
...
oxford_locations <- readr::read_csv(paths$oxford_file, col_types = "c")
...
# Area
aoi <- read_sf(file.path(user_input_dir, "AOI")) %>%
...
koeppen <- read_csv(paths$koeppen_file, col_types = "ddc")
...
# city_params$bm_cities_manual <- c('Luanda', 'Kinshasa', 'Libreville', 'Yaound�', 'Bangui', 'N\'Djamena', 'Abuja', 'Juba')
...
pop_manual <- read_csv(file.path(user_input_dir, "manual-data-entry/pop.csv"), col_types = "ccddc") %>% mutate(Method = "Manual")
...
if (in_oxford) pop_growth <- bind_rows(pop_growth, get_de_pop_growth(city, country))

# Write pop_growth to data-sharing
write_csv(pop_growth, file.path(data_sharing_dir, "04-tabular-data/population-growth.csv"))
# 
# pop_capital <- read_xls(paths$pop_capital_file, skip = 16)
# pop_capital %>% filter(str_detect(`Country or area`, city_params$nearby_countries_string))
...
paste0(
  ...
  scales::percent(pct_growth, big.mark = ","),
  " from ", scales::label_comma()(pop_1), " in ", year_1, " to ", scales::comma(pop_last), " in ", year_last,
  ", at an average annual rate of ", scales::percent(avg_growth, accuracy = 0.1)) %>%
  print_text()
...
```{r}
# WorldPop Age Groups
age_file <- fuzzy_read(stats_dir, "demographics.csv", FUN = paste)
...
### Urban built-up area
```{r}
wsf_stats_file <- fuzzy_read(stats_dir, "wsf_stats.csv", FUN = paste)
if (is.na(wsf_stats_file)) {
  warning("No WSF.xlsx file. Should there be? Check for different file name.")
} else {
  # Urban built-up area ----
  wsf <- read_csv(wsf_stats_file, col_types = "id") %>%
  ...
  print_paged_df(wsf)
  write_csv(wsf, file.path(data_sharing_dir, "/04-tabular-data/urban-extent-and-change.csv"))
...
### Landcover
```{r}
lc_file <- fuzzy_read(stats_dir, "landcover.csv")
lc <- read_xlsx(lc_file, skip = 1,
...
### Elevation
```{r}
elevation_file <- fuzzy_read(stats_dir, "elevation.csv")
elevation <- read_csv(elevation_file) 
...
### Slope
```{r}
slope_file <- fuzzy_read(stats_dir, "slope.csv")
slope <- read_csv(slope_file) 
...
flood_string <- function(flood_type) {
  osmpt <- read_xls(file.path(stats_dir, paste0(flood_type, "_osmpt.xls")))[,1:4] %>%
    distinct() %>%
    tryCatch(error = function(e) {
      warning(paste0("File `", stats_dir, "/", flood_type, "_osmpt.xls` does not exist. Is it named differently?<br><br>"))
      return(NULL)
    })
  major_roads <- read_xls(file.path(stats_dir, paste0(flood_type, "_majorroad.xls"))) %>%
...
gather_flood_data <- function(flood_type) {
  ...
  df <- read_xlsx(file.path(stats_dir, "WSF.xlsx"), sheet = toupper(flood_type)) %>%
...
flood_pop_area <- function(flood_type) {
  read_xls(file.path(stats_dir, paste0(flood_type, "_pop_area.xls"))) %>% 
    arrange(VALUE) %>% 
    rename(Area = VALUE_1) %>%
    # {.$Area[2] / .$Area[1]}
...
### Fluvial
```{r}
if (!file.exists(file.path(stats_dir, "WSF.xlsx")) warning("No WSF.xlsx file.") else {
...
c("dbf", "prj", "shp", "shx") %>% lapply(function(suffix) {
  curl_download(
    ...
    destfile = paste0(paths$flood_archive_file, "/FloodArchive_region.", suffix))
})

flood_archive <- read_sf(paths$flood_archive_file)
...
st_write(flood_archive, file.path(data_sharing_dir, "03-analysis-inputs/flood-events/flood-events.shp"), append = F)
```
...
st_write(eq_near, file.path(data_sharing_dir, "03-analysis-inputs/earthquakes/earthquakes-500km.shp"), append = F)
...
cyclones <- read_sf(paths$cyclone_archive_file) %>%
  st_transform("EPSG:4326")
# plot(cyclones)

cyclones_near_index <- st_is_within_distance(aoi, cyclones, as_units(500, "km"))
...
if (nrow(cyclones_near) > 0) {
  ...
  cyclones_text %>% filter(min_distance <50) %>% count(USA_SSHS)  %>% print_paged_df()
}
```
