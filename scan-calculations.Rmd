...
elevation2 <- elevation %>% subset(!is.na(Bin)) %>%
  mutate(Percent = percent,
         Elevation = as.numeric(str_replace(Bin, "-.*", "")),
...
```{r}
# Set parameters
aoi <- vect("/Users/bennotkin/Downloads/AOI 5") %>% project("EPSG:4326")
fwi_directory <- "source-data.nosync/nasa-fwi/" # Include final forward slash
dates <- as.Date("2016-01-01"):as.Date("2021-12-31") %>% as.Date() # We have data for 2016-01-01 through 2022-09-04
city <- "Bor"
