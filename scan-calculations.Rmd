...
### Population Density
```{r}
density <- pop_longitude %>% slice_max(order_by = Year, by = Location)
...
```{r}
# WorldPop Age Groups
age_file <- list.files("Stats")[!is.na(str_match(list.files("Stats"), ".*age_distribution.csv"))]
if (length(age_file) == 0) warning("No WorldPop age distribution file") else {
  pop_dist_group_wp <- read_csv(paste0("Stats/", age_file), col_types = "fdd") %>% 
