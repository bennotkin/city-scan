...
bm_cities <- oxford_full %>%
  select(Location, Country, Indicator, `2021`) %>%
  subset(Location %in% nearby_cities & Indicator == "Total population") %>%
  subset((between(`2021`, pop*.66, pop*1.33)) & Location != city) %>%
  .$Location
...
write_csv(select(pop_growth), "data-sharing/04-tabular-data/population-growth.csv")
...
cyclones_text_select

cyclones_text %>% filter(min_distance <50) %>% count(USA_SSHS)
```
