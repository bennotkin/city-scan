...
write_csv(density, "data-sharing/04-tabular-data/population-density.csv")
...
if (in_oxford) {
  ...
  write_csv(pop_dist_group, "data-sharing/04-tabular-data/oxford-pop-distro.csv")
...
```{r}
# WorldPop Age Groups
age_file <- list.files("tabular")[!is.na(str_match(list.files("tabular"), ".*_demographics.csv"))]
if (length(age_file) == 0) warning("No WorldPop age distribution file") else {
  pop_dist_group_wp <- read_csv(paste0("tabular/", age_file), col_types = "ffd") %>% 
  ...
  write_csv(pop_dist_group, "data-sharing/04-tabular-data/world-pop-age-sex.csv")
...
if (in_oxford) {
  ...
  write_csv(pop_dist_structure, "data-sharing/04-tabular-data/oxford-age-structure-area.csv")
...
if (in_oxford) {
  ...
  write_csv(national_shares_long, "data-sharing/04-tabular-data/oxford-national-shares.csv")
}
```
...
write_csv(pop_longitude, "data-sharing/04-tabular-data/oxford-pop_line.csv")
...
if (in_oxford) {
  ...
  ggsave("plots/oxford-emp_line.png", device = "png", plot = multi_city_emp_plot,
         width = 5, height = 4.6, units = "in", dpi = "print")
  write_csv(emp_longitude, "data-sharing/04-tabular-data/oxford-emp_line.csv")
}
```
...
scale_y_continuous(
  ...
  ggsave("plots/oxford-gdp_line.png", device = "png",
         width = 5, height = 4.6, units = "in", dpi = "print")
  write_csv(gdp_longitude, "data-sharing/04-tabular-data/oxford-gdp_line.csv")
}
```
...
if (in_oxford) {
  ...
  ggsave("plots/oxford-gdp-per-capita_line.png", gdppc_longitude_plot, device = "png",
         width = 8, height = 6.5, units = "in", dpi = "print")
  write_csv(gdppc_longitude, "data-sharing/04-tabular-data/oxford-gdp-per-capita_line.csv")
}
```
...
if (in_oxford) {
  ...
  write_csv(emp_shares2, "data-sharing/04-tabular-data/oxford-employment-sectors.csv")
}
```
...
if (in_oxford) {
  ...
  ggsave("plots/oxford-gva-sectors.png", plot = gva_shares_plot, device = "png",
         width = 16, height = 5, units = "in", dpi = "print")
  write_csv(gva_shares2, "data-sharing/04-tabular-data/oxford-gva-sectors.csv")
}
```
...
if (in_oxford) {
  ...
  write_csv(household_incomes, "data-sharing/04-tabular-data/oxford-household-incomes-bar.csv")
  ...
  ggsave("plots/oxford-household-incomes-area.png", plot = household_incomes_area_plot, device = "png",
         ...
         width = 4, height = 3.5, units = "in", dpi = "print")
  write_csv(incomes_city2, "data-sharing/04-tabular-data/oxford-household-incomes-area.csv")
}
```
...
### Urban built-up area
```{r}
wsf_file <- str_subset(list.files("tabular", full.names = T), "(?<!flood_)wsf(?!.*tracker).*(csv|xlsx)") 
...
```{r}
wsf_tracker_file <- str_subset(list.files("tabular", full.names = T), "wsf_tracker.*(csv|xlsx)") 
...
### Landcover
```{r}
lc <- read_csv(str_subset(list.files("tabular", full = T), "lc.csv"), col_types = "cd") %>%
...
ggsave("plots/wsf-lc-pieplot.png", plot = lc_plot, device = "png",
       width = 8, height = 5, units = "in", dpi = "print")
write_csv(lc, "data-sharing/04-tabular-data/landcover.csv")
...
### Elevation
```{r}
elevation <- read_csv(str_subset(list.files("tabular", full = T), "elevation.csv"), col_types = "cd") %>%
  subset(!is.na(Bin)) %>%
...
ggsave("plots/wsf-elevation-pieplot.png", device = "png",
       width = 8, height = 5, units = "in", dpi = "print")
write_csv(elevation, "data-sharing/04-tabular-data/elevation.csv")
```

### Slope
```{r}
slope <- read_csv(str_subset(list.files("tabular", full = T), "slope.csv"), col_types = "cd") %>%
  subset(!is.na(Bin)) %>%
...
ggsave("plots/wsf-slope-pieplot.png", device = "png",
       width = 8, height = 5, units = "in", dpi = "print")
write_csv(slope, "data-sharing/04-tabular-data/slope.csv")
```

## Urban Flooding
```{r}
flood_osm_file <- list.files("tabular", full.names = T) %>% str_subset("flood_osm")
...
flood_road_file <- list.files("tabular", full.names = T) %>% str_subset("flood_road")
...
flood_file <- str_subset(list.files("tabular", full.names = T), "flood_wsf.csv")
...
flood_pop_file <- list.files("tabular", full.names = T) %>% str_subset("flood_pop")
if (length(flood_road_file) == 1) {
...
if (length(wsf_file) == 0 || !file.exists(wsf_file)) warning("No WSF.xlsx file.") else {
  ...
  ggsave("plots/wsf-fu-plot.png", plot = fu_plot, device = "png",
         width = 4, height = 3.5, units = "in", dpi = "print")
  write_csv(fu, "data-sharing/04-tabular-data/wsf-fluvial.csv")
...
if (length(wsf_file) == 0 || !file.exists(wsf_file)) warning("No WSF.xlsx file.") else {
  ...
  ggsave("plots/wsf-pu-plot.png", plot = pu_plot, device = "png",
         width = 4, height = 3.5, units = "in", dpi = "print")
  write_csv(pu, "data-sharing/04-tabular-data/wsf-pluvial.csv")
...
if (length(wsf_file) == 0 || !file.exists(wsf_file)) warning("No WSF.xlsx file.") else {
  ...
  ggsave("plots/wsf-cu-plot.png", plot = cu_plot, device = "png",
         width = 4, height = 3.5, units = "in", dpi = "print")
  write_csv(cu, "data-sharing/04-tabular-data/wsf-coastal.csv")
...
if (length(wsf_file) == 0 || !file.exists(wsf_file)) warning("No WSF.xlsx file.") else {
  ...
  ggsave("plots/wsf-pufu-plot.png", plot = pufu_plot, device = "png",
         width = 4, height = 3.5, units = "in", dpi = "print")
  write_csv(pufu, "data-sharing/04-tabular-data/wsf-combined-flooding.csv")
...
write_csv(flood_text_all, "data-sharing/04-tabular-data/flood-events.csv")
```
...
eq_text %>%
  st_drop_geometry() %>%
  mutate(
    distance = as.vector(units::drop_units(distance)),
    magXdist = as.vector(magXdist)) %>%
  write_csv("data-sharing/04-tabular-data/earthquake-events.csv")
```
...
if (nrow(cyclones_near) > 0) {
  ...
  write_csv(cyclones_text, "data-sharing/04-tabular-data/cyclone-events.csv")
}
```
...
ggsave("plots/monthly-pv.png", device = "png",
       width = 4, height = 3.5, units = "in", dpi = "print")
write_csv(monthly_pv, "data-sharing/04-tabular-data/monthly-pv.csv")
```
...
fwi_file <- str_subset(list.files("tabular", full = T), "fwi")
if (length(fwi_file) == 0) warning("No FWI file") else {
  ...
  ggsave(file.path("plots/nasa-fwi.png"), plot = fwi_plot, device = "png",
        # width = 4, height = 3.5, units = "in", dpi = "print")
         width = 8, height = 6.5, units = "in", dpi = "print")
  write_csv(fwi, "data-sharing/04-tabular-data/fwi.csv")
}
```
