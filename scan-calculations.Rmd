...
elevation2 <- elevation %>% subset(!is.na(Bin)) %>%
  mutate(Elevation = as.numeric(str_replace(Bin, "-.*", "")),
         legend = factor(Bin, levels = Bin))
...
elevation_names <- elevation2$legend
elevation_colors <- c(
  "#f5c4c0",
  "#f19bb4",
  "#ec5fa1",
...
### Slope
```{r}
slope <- read_csv(str_subset(list.files("Stats", full = T), "slope"), col_types = "cdd") %>%
  rename(legend = Bin)

slope2 <- slope %>% subset(!is.na(legend)) %>%
  mutate(Slope = as.numeric(str_replace(legend, "[-+].*", "")),
...
gather_flood_data <- function(flood_type) {
  wsf_file <- str_subset(list.files("Stats", full.names = T), "WSF") 
  df <- read_xlsx(wsf_file, sheet = toupper(flood_type)) %>%
...
```{r}
# Set parameters
dates <- as.Date(as.Date("2016-01-01"):as.Date("2021-12-31")) # We have data for 2016-01-01 through 2022-09-04
...
extract_aoi <- function(aoi, dates) {
  ...
  cord <- dates %>% # cord is a bad fire pun
    lapply(function(date) {
      file_name <- file.path(fwi_directory, paste0("FWI.GEOS-5.Daily.Default.", date %>% stringr::str_replace_all("-", ""), ".tif"))
...
writeRaster(q99_raster, paste0(city, "-fwi-98th.tif"))
...
{  bind_rows(
    ...
    summarize(., .by = c(week), q90 = quantile(FWI, .95), year = "All")
  ) } %>%
