...
get_de_pop_growth <- function(city, country = country) {
  ...
  for (id in table_ids) {
    de <- read_html(url) %>%
      html_node(paste0("section#", table_id)) %>%
      html_node("table") %>%
      html_table()
...
### Urban built-up area
```{r}
wsf_file <- str_subset(list.files("Stats", full.names = T), "(?<!flood_)wsf(?!.*tracker).*(csv|xlsx)") 
...
flood_file <- str_subset(list.files("Stats", full.names = T), "flood.*csv")
wsf_flood <- full_join(wsf, read_csv(flood_file), by = c("Year" = "year")) %>%
  rowwise() %>%
  mutate(combined_2020 = sum(c_across(contains("2020")))) %>% ungroup()
...
if (length(wsf_file) == 0 || !file.exists(wsf_file)) warning("No WSF.xlsx file.") else {
  ...
  print_text(glue("In {comb$Year[1]}, {round(comb$uba_km2_exposed[1], 2)} km^2 of the built-up area ({comb$percent_uba_exposed[1]}) was exposed to river flooding at a minimum depth of 15 cm"))
