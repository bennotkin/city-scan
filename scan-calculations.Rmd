...
in_oxford <- city %in% oxford_locations$Location
if (!in_oxford) {
...
### Urban built-up area
```{r}
wsf_file <- str_subset(list.files("Stats", full.names = T), "wsf(?!.*tracker).*(csv|xlsx)") 
...
```{r}
wsf_tracker_file <- str_subset(list.files("Stats", full.names = T), "wsf_tracker.*(csv|xlsx)") 
if (!file.exists(wsf_tracker_file)) {
  ...
  if (str_detect(wsf_tracker_file, "csv$")) wsf_tracker <- read_csv(wsf_tracker_file, col_types = "cd")
  ...
  uba_tracker_plot <- wsf_tracker %>%
    ...
    #   breaks = seq(1985, 2020, 5),
    #   minor_breaks = seq(1985, 2021, 1)) + 
    scale_y_continuous(labels = scales::comma, limits = c(0, max(wsf_tracker$uba_km2)), expand = c(0, NA)) +
    theme_minimal() +
    labs(title = "",#paste(city, "Urban Built-up Area, 1985-2015"),
         y = bquote('Urban built-up area,'~km^2)) +
    theme(axis.line = element_line(linewidth = .5, color = "black"))
  ggsave("plots/wsf-uba-tracker-plot.png", plot = uba_tracker_plot, device = "png",
         width = 4, height = 3.5, units = "in", dpi = "print")
  ...
  write_csv(wsf_tracker, "data-sharing/04-tabular-data/urban-extent-and-change-tracker.csv")
}
```
...
flood_string <- function(flood_type) {
  ...
  if (is.null(osmpt)) return("No OSMPT data.") else {
    roads_pct <- (major_roads$ratio[major_roads$gridcode == 1] * 100) %>% round(0)
    ...
    flood_type_long <- str_replace_all(flood_type,
      c(fu = "riverine",
      pu = "surface water",
      cu = "coastal",
...
gather_flood_data <- function(flood_type) {
  ...
  df <- 
    tryCatch(
      read_excel(wsf_file, sheet = toupper(flood_type)) %>%
        select(Year = Value, uba_km2 = AREA_sq_km),
      error = \(e) {
        warning(glue('Error on gather_flood_type_data("{flood_type}"): {e}'))
        no_data_df <- tibble(Year = wsf$Year, uba_km2 = 0)
        return(no_data_df)
      })
...
flood_pop_area <- function(flood_type) {
  ...
  tryCatch(
    read_excel(pop_file) %>% 
      ...
      error = \(e) {
        warning(glue('Error on flood_pop_area("{flood_type}"): {e}'))
        no_data_df <- tibble(Percentage = 0)
        return(no_data_df)
      })
}
```
...
eq_plot <-
  ...
  geom_point(data = \(x) filter(x, !is.na(eqMagnitude)), aes(size = eqMagnitude), shape = 1) +
  geom_point(data = \(x) filter(x, is.na(eqMagnitude)), shape = 1, aes(color = "grey92")) +
  scale_x_continuous(
    limits = c(0, 500),
    labels = \(x) paste(x, "km"),
  ...
  theme(
    ...
    plot.margin = margin(5.5, 300, 5.5, 5.5, "pt"),
    plot.background = element_rect(fill = "white", color = NULL))
  ggsave("plots/ngdc-earthquake-scatter.png", plot = eq_plot, device = "png",
         width = 20, height = 5.833, units = "in", dpi = "print")
...
```{r}
files <- list.files(pv_path, full.names = T) %>% 
  subset(stringr::str_detect(., ".tif$|.asc$"))

aoi_largest <- vect("AOI") %>% .[which.max(expanse(.)),,] %>% project("EPSG:4326")
aoi_ext <- ext(aoi_largest)

all_months <- lapply(files, \(f) rast(f, win = aoi_ext, snap = "out")) %>% purrr::reduce(c)
monthly_pv_wide <- extract(all_months, aoi_largest, weights = T, fun = mean)
monthly_pv <- t(monthly_pv_wide[,-1]) %>%
...
monthly_pv %>%
  # Data for some countries is in format of month totals
  # mutate(mean = mean/lubridate::days_in_month(month)) %>%
...
if (length(fwi_file) == 0) warning("No FWI file") else {
  ...
  ggsave(file.path("plots/nasa-fwi.png"), plot = fwi_plot, device = "png",
        width = 4, height = 3.5, units = "in", dpi = "print")
}
```
