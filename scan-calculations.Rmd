...
c("plots", "data-sharing", "data-sharing/02-TIFs",
...
file.copy(from = "AOI/", to = "data-sharing", recursive = T)
...
if (!exists("oxford_full")) oxford_full <-
  ...
  # mutate(Location = case_when(Location == "Denpasar" ~ "Sarbagita", T ~ Location))
...
nearby_countries_string <- paste0(nearby_countries_string, "|", tolower(country))
...
pop_manual <- read_csv("manual-data-entry/pop.csv", col_types = "ccddc") %>% mutate(Method = "Manual")
...
write_csv(pop_growth, "data-sharing/04-tabular-data/population-growth.csv")
...
pmap(pop_growth_summary, \(...) {
  row <- tibble(...)
  glue("{row$Location}'s population grew by {row$change_pct} from {scales::comma(row$pop_1)} in {row$year_1} to {scales::comma(row$pop_last)} in {row$year_last}")
}) %>% unlist() %>% cat(sep = "\n") %>% print_text()
...
```{r}
# WorldPop Age Groups
age_file <- list.files("Stats")[!is.na(str_match(list.files("Stats"), ".*_demographics.csv"))]
if (length(age_file) == 0) warning("No WorldPop age distribution file") else {
  pop_dist_group_wp <- read_csv(paste0("Stats/", age_file), col_types = "ffd") %>% 
...
multi_city_pop_plot <- ggplot(pop_longitude) +
  ...
  scale_y_continuous(limits = c(0, NA), labels = scales::label_comma(), expand = c(0,0)) +
  scale_x_continuous(breaks = seq(min(pop_longitude$Year), 2021, 5), minor_breaks = seq(2000,2030,1), expand = c(0,0)) +
  # geom_dl(aes(label = Location), method = list(dl.combine("first.points", "last.points")), cex = 0.8) 
  geom_text_repel(data = pop_longitude %>% slice_max(order_by = Year, by = Location), aes(label = Location, x = 2021, y = `Population`, color = Group), 
...
if (in_oxford) {
  ...
  multi_city_gdp_plot <- ggplot(gdp_longitude) +
    ...
    scale_y_continuous(limits = c(0, NA), labels = scales::label_dollar(), expand = c(0,0)) +
    scale_x_continuous(
...
### Urban built-up area
```{r}
wsf_file <- str_subset(list.files("Stats", full.names = T), "wsf.*(csv|xlsx)") 
if (!file.exists(wsf_file)) {
  warning("No WSF.xlsx file. Should there be? Check for different file name.")
} else {
  # Urban built-up area ----
  wsf <- if (str_detect(wsf_file, "xlsx$")) read_xlsx(wsf_file, sheet = "WSF")
  wsf <- if (str_detect(wsf_file, "csv$")) read_csv(wsf_file, col_types = "dd")
  wsf <- wsf %>%
    rename(Year = 1) %>%
  ...
  print_paged_df(wsf)
  write_csv(wsf, "data-sharing/04-tabular-data/urban-extent-and-change.csv")
...
### Landcover
```{r}
lc <- read_csv(str_subset(list.files("Stats", full = T), "lc"), col_types = "cd") %>%
...
### Elevation
```{r}
elevation <- read_csv(str_subset(list.files("Stats", full = T), "elevation"), col_types = "cd") %>%
  subset(!is.na(Bin)) %>%
...
### Slope
```{r}
slope <- read_csv(str_subset(list.files("Stats", full = T), "slope"), col_types = "cd") %>%
  subset(!is.na(Bin)) %>%
...
flood_string <- function(flood_type) {
  osmpt_file <- str_subset(list.files("Stats", full.names = T), paste0(flood_type, "_osmpt.xls")) 
  osmpt <- read_excel(osmpt_file)[,1:4] %>% distinct() %>%
    tryCatch(error = function(e) {
      warning(paste0("File `Stats/", flood_type, "_osmpt.xls does not exist. Is it named differently?<br><br>"))
      return(NULL)
    })
  major_roads_file <- str_subset(list.files("Stats", full.names = T), paste0(flood_type, "_majorroad.xls")) 
  major_roads <- read_excel(major_roads_file) %>%
    tryCatch(error = function(e) return(data.frame(OID = 0, gridcode = 0, LENGTH = 0, PERCENTAGE = 0)))
  
  if (is.null(osmpt)) return("No OSMPT data.") else {
    roads_pct <- major_roads$PERCENTAGE[major_roads$gridcode == 1] %>% round(0)
    police_ct_flood <- osmpt$PNT_COUNT[osmpt$source == "osm_police" & osmpt$gridcode == 1]
    police_ct <- osmpt$PNT_COUNT[osmpt$source == "osm_police" & osmpt$gridcode == 0]
    health_ct_flood <- osmpt$PNT_COUNT[osmpt$source == "osm_health" & osmpt$gridcode == 1]
    health_ct <- osmpt$PNT_COUNT[osmpt$source == "osm_health" & osmpt$gridcode == 0]
    school_ct_flood <- osmpt$PNT_COUNT[osmpt$source == "osm_schools" & osmpt$gridcode == 1]
    school_ct <- osmpt$PNT_COUNT[osmpt$source == "osm_schools" & osmpt$gridcode == 0]
    fire_ct_flood <- osmpt$PNT_COUNT[osmpt$source == "osm_fire" & osmpt$gridcode == 1]
    fire_ct <- osmpt$PNT_COUNT[osmpt$source == "osm_fire" & osmpt$gridcode == 0]
    ...
    string <- paste0(
      ...
      school_ct_flood, " of ", school_ct, " schools, ", 
      health_ct_flood, " of ", health_ct, " health facilities, ", 
      fire_ct_flood, " of ", fire_ct, " fire stations, and ", 
      police_ct_flood, " of ", police_ct, " police stations are located in a ", 
...
gather_flood_data <- function(flood_type) {
  wsf_file <- str_subset(list.files("Stats", full.names = T), "wsf.*xlsx") 
  df <- read_excel(wsf_file, sheet = toupper(flood_type)) %>%
    select(Year = Value, uba_km2 = AREA_sq_km)
  df <- full_join(df, wsf, by = "Year", suffix = c("_exposed", "_wsf")) %>%
    select(-growth_pct, -growth_km2) %>%
    arrange(Year) %>%
    fill(uba_km2_exposed) %>%
    mutate(
      growth_pct = scales::percent(uba_km2_exposed / lag(uba_km2_exposed) - 1, accuracy = 0.01),
      percent_uba_exposed = scales::percent(uba_km2_exposed/uba_km2_wsf, accuracy = 0.01))
  return(df)
}
flood_pop_area <- function(flood_type) {
  pop_file <- str_subset(list.files("Stats", full.names = T), paste0(flood_type, "_pop_area.xls")) 
  read_excel(pop_file) %>% 
    arrange(VALUE) %>% 
    rename(Area = VALUE_1) %>%
    # {.$Area[2] / .$Area[1]}
    mutate(Total_Pop_Area = max(Area),
           Percentage = Area/Total_Pop_Area,
           Percentage = scales::percent(Percentage, accuracy = 0.01)) %>%
    filter(VALUE == 1) %>%
...
if (length(wsf_file) == 0 || !file.exists(wsf_file)) warning("No WSF.xlsx file.") else {
  ...
  print_text(paste0("In ", fu[1,1], ", ", round(fu[1,2], 2), " km^2 of the built-up area was exposed to river flooding at a minimum depth of 15 cm"))
  print_text(paste("Percentage of highest populated area exposed to fluvial flooding:", flood_pop_area("fu")$Percentage))
}
...
if (length(wsf_file) == 0 || !file.exists(wsf_file)) warning("No WSF.xlsx file.") else {
  ...
  print_text(paste0("In ", pu[1,1], ", ", round(pu[1,2], 2), " km^2 of the built-up area was exposed to surface water flooding at a minimum depth of 15 cm"))
  print_text(paste("Percentage of highest populated area exposed to pluvial flooding:", flood_pop_area("pu")$Percentage))
...
```{r}
wsf_file <- str_subset(list.files("Stats", full.names = T), "wsf.*xlsx")
if (!file.exists(wsf_file)) warning("No WSF.xlsx file.") else {
  cu <- gather_flood_data("cu")
  cu_plot <- cu %>%
    ggplot +
    geom_line(aes(x = Year, y = uba_km2_exposed)) +
    scale_x_continuous(
  ...
  ggsave("plots/wsf-cu-plot.png", plot = cu_plot, device = "png",
         width = 4, height = 3.5, units = "in", dpi = "print")
  
  print_paged_df(bind_rows(head(cu, 1), tail(cu, 1)))
  print_text(paste0("In ", cu[1,1], ", ", round(cu[1,2], 2), " km^2 of the built-up area was exposed to coastal flooding at a minimum depth of 15 cm"))
  print_text(paste("Percentage of highest populated area exposed to coastal flooding:", flood_pop_area("cu")$Percentage))

}
print_text(flood_string("cu"))
```
...
if (!file.exists(wsf_file)) warning("No WSF.xlsx file.") else {
  ...
  print_text(paste0("In ", comb[1,1], ", ", round(comb[1,2], 2), " km^2 of the built-up area was exposed to river or surface water flooding at a minimum depth of 15 cm"))
  print_text(paste("Percentage of highest populated area exposed to combined flooding:", flood_pop_area("comb")$Percentage))
}
...
st_write(flood_archive, "data-sharing/03-analysis-inputs/flood-events/flood-events.shp", append = F)

# ggplot(flood_text_all) +
#   # geom_linerange(aes(xmin = BEGAN, xmax = ENDED), y = 0) +
#   geom_point(aes(x = BEGAN, y = 0, size = SEVERITY)) +
#     scale_x_date(limits = c(as.Date("1984-01-01"),as.Date("2020-12-31")), expand = c(0,0)) +
#     scale_y_continuous(limits = c(-1800, 1800), breaks = c(1825,3650)) +
#     theme_minimal()
#   theme(axis.text = element_blank(), axis.title = element_blank(), panel.grid = element_blank())

flood_text_all2 <- flood_text_all %>%
  mutate(
...
sort(flood_text_all2$mag, decreasing = T)[10]

# flood_text
# flood_text <- flood_text_all %>%
#     mutate(mag = normalize(DEAD) * normalize(DISPLACED) * SEVERITY) %>%
#     slice_max(mag, n = 10)

# flood_text_all2[which(order(flood_text_all2$mag, decreasing = TRUE) > 10),"text"] <- NA
flood_text_all2[which(rank(-flood_text_all2$mag) >= 10),"text"] <- NA
...
flood_text_all2 %>%
  ...
  # geom_label_repel(
  #   aes(label = text),
  #   size = 2, min.segment.length = 0, label.r = 0, alpha = 0.8, force_pull = .25, box.padding = .5) +
  scale_x_continuous(
  ...
  scale_size_continuous(
    range = c(3, 10),
    breaks = range(flood_text_all2$DISPLACED),
    labels = scales::label_number(scale_cut = scales::cut_short_scale())) +
  labs(alpha = "Severity", size = "Displaced")
ggsave("plots/darthmouth_flood_events_alt.pdf", device = "pdf",
       width = 20, height = 5.833, units = "in", dpi = "print")
```
...
if (nrow(eq_text) > 0) {
  print_text("Significant earthquakes within 500 km of city")
  print_paged_df(eq_text) # For some reason errors if 0 rows
  
  eq_text %>% sf::st_drop_geometry() %>%
  mutate(distance_round = ceiling(distance/100) * 100) %>%
  count(distance_round) %>% arrange(distance_round) %>%
  mutate(cumsum = cumsum(n)) %>% print_paged_df()
} else {
  "No significant earthquakes within 500 km"
}
...
st_write(eq_near, "data-sharing/03-analysis-inputs/earthquakes/earthquakes-500km.shp", append = F)
...
eq_text_select <- eq_text_select %>% mutate(text2 = glue("{line1}; {line2}; {line3}; {line4}"))
eq_plot <- ggplot(mapping = aes(y = BEGAN, x = drop_units(distance))) +
  geom_point(data = filter(eq_text, !is.na(eqMagnitude)), aes(size = eqMagnitude), shape = 1) +
  # geom_text(data = filter(eq_text, !is.na(eqMagnitude)), aes(size = eqMagnitude, label = eqMagnitude)) +
  geom_text_repel(data = eq_text_select, aes(label = text2), size = 3,
                  direction = "y", hjust = 0, xlim = c(505, 1000),
                  segment.linetype = "dotted") +
  geom_point(data = filter(eq_text, is.na(eqMagnitude)), shape = 1, aes(color = " ")) +
  scale_x_continuous(
    limits = c(0, 500),
  ...
  ggsave("plots/earthquake-scatter.png", plot = eq_plot, device = "png",
         width = 9, height = 6.5, units = "in", dpi = "print")
  ggplot2:::print.ggplot(eq_plot)
```
...
if (nrow(cyclones_near) > 0) {
  ...
  ggplot(cyclones_text) +
    ...
    theme(axis.text = element_blank(), axis.title = element_blank(), panel.grid = element_blank()) +
    force_panelsizes(rows = unit(5.833, "in"), cols = unit(20, "in"))
  ggsave("plots/ncei-cyclones.pdf", device = "pdf",
         width = 20, height = 5.833, units = "in", dpi = "print")
...
monthly_pv <- lapply(files, function(f) {
  ...
  month_country <- terra::rast(paste0(pv_path, f))
...
fwi_file <- str_subset(list.files("Stats", full = T), "fwi")
if (length(fwi_file) == 0) warning("No FWI file") else {
  ...
  ggplot(fwi, aes(x = week, y = pctile_95)) +
    ...
    theme(axis.line = element_line(linewidth = .5, color = "black"),
          ...
          panel.grid.minor = element_line(linewidth = .125, linetype = 2, color = "dark gray"),
          axis.title.x = element_blank())
  ggsave(file.path("plots/nasa-fwi.png"), device = "png",
        width = 4, height = 3.5, units = "in", dpi = "print")
}
```
